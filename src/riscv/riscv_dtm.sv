...
module riscv_dtm #(
    ...
    // ========================================================================
    // DMI Register (40 bits for abits=6: 6 addr + 32 data + 2 op)
    // ========================================================================
    // Bits [39:34]: address (6 bits, matches abits=6)
    ...
    logic [39:0] dmi_shift;
    logic        bypass_shift;

    // DMI state
    logic [ 5:0] dmi_address;
...
`endif
                ...
                IR_DMI: begin
                    ...
                    case (dmi_address)
                        6'h10:   dmi_shift <= {dmi_address, dmcontrol, 2'b00};       // dmcontrol
                        6'h11:   dmi_shift <= {dmi_address, DMSTATUS_VALUE, 2'b00};  // dmstatus
                        6'h16:   dmi_shift <= {dmi_address, HARTINFO_VALUE, 2'b00};  // hartinfo
...
`ifdef VERBOSE
                    $display("[%0t] DTM: SHIFT_DR IDCODE, tdo=%b, idcode_shift=%h -> %h", $time, idcode_shift[0],
                             idcode_shift, {tdi_i, idcode_shift[31:1]});
`endif
                end
                IR_DTMCS:  dtmcs_shift <= {tdi_i, dtmcs_shift[31:1]};
                IR_DMI:    dmi_shift <= {tdi_i, dmi_shift[39:1]};
    ...
    always_ff @(posedge tck_i or negedge ntrst_i) begin
        if (!ntrst_i) begin
            ...
            dmi_address <= dmi_shift[39:34];

            // Process write operations
            if (dmi_shift[1:0] == 2'b10) begin  // Write operation
                case (dmi_shift[39:34])
                    6'h10: dmcontrol <= dmi_shift[33:2];  // Write to dmcontrol
                    default: begin
